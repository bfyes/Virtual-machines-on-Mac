# Vivado（含可用的Batch模式） 2022.2 on Apple Silicon Mac（不完善）

最后更新时间：2026-03-26

注：笔者能力十十十十十分有限，只能写到这个程度了。。。如果以后能力有提升会完善这篇文章的。

另注：如果您很需要，可以联系我，直接copy一份虚拟机给你（）

> 本文的部署流程是通过AI辅助完成的，总结这篇文章也是为了记录在 linux 系统里的“摸爬滚打”。这篇文章**没有任何技术含量**，只是记录我装机折腾的种种过程。毕竟跟谁学都是学，包括AI。现在列出步骤只是为了说明该方案的可行性及便捷性。笔者刚刚接触 linux，是小白，实在不熟悉这个系统的环境和操作。步骤只是大致的框架，如遇到任何问题，请做“Prompt工程师”。
...
- **这个步骤可能是文章唯一给出的<span style="color:#FF0000;">有用信息</span>hhh。**
  ...
  废话不多说，上链接：<https://macwk.cn/app/2941.html>。
  
  > [!Warning]
  >
  > <span style="color:#FF0000;">**叠甲**</span> 仅供学习使用！请在24小时之内删除。请支持正版软件。
  
  如果签名失败，请先将安装容器的所有文件复制并粘贴到一个新建文件夹中，然后执行：
...
<img src="Vivado(Batch" alt="1Capture 2026-03-26 18.15.01" /> on Apple Silicon Mac.assets/1Capture 2026-03-26 18.15.01-1774520136604-2.png)
...
### 第 2 步：配置 RosettaLinux 的 binfmt 路由
...
### 第 5 步：修改 `xsetup` 架构检查

只改“报错退出”的架构判断分支，其他安装逻辑不要动。仅需把exit1前面加`#`注释掉即可。图片中是AI修改后的片段，不必这么麻烦。

<img src="Vivado(Batch" alt="image-20260326184049178" /> on Apple Silicon Mac.assets/image-20260326184049178.png)
...
如不是 `x11`，优先回到第 6.5 步切换 Xorg 后再测 GUI。

<img src="Vivado(Batch" alt="image-20260326185039563" /> on Apple Silicon Mac.assets/image-20260326185039563.png)
